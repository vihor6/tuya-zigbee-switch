...
# ==============================================================================
SILABS_SDK_LINES := simplicity_sdk gecko_sdk

define silabs_sdk_version
$(if $(filter simplicity_sdk,$(1)),2025.6.2,$(if $(filter gecko_sdk,$(1)),4.5.0,))
endef

define silabs_sdk_archive
$(if $(filter simplicity_sdk,$(1)),simplicity-sdk.zip,$(if $(filter gecko_sdk,$(1)),gecko-sdk.zip,))
endef

define silabs_sdk_url
$(if $(filter simplicity_sdk,$(1)),https://github.com/SiliconLabs/simplicity_sdk/releases/download/v2025.6.2/simplicity-sdk.zip,$(if $(filter gecko_sdk,$(1)),https://github.com/SiliconLabs/gecko_sdk/releases/download/v4.5.0/gecko-sdk.zip,))
endef

define silabs_sdk_line_for_mcu
$(if $(filter EFR32MG13P732F512GM48,$(1)),gecko_sdk,$(if $(filter EFR32MG21A020F768IM32 EFR32MG22C224F512GN32,$(1)),simplicity_sdk,))
endef

SILABS_SDK_LINE := $(call silabs_sdk_line_for_mcu,$(MCU))
SILABS_SDK_VERSION := $(call silabs_sdk_version,$(SILABS_SDK_LINE))
SILABS_SDK_ARCHIVE := $(call silabs_sdk_archive,$(SILABS_SDK_LINE))
SILABS_SDK_URL := $(call silabs_sdk_url,$(SILABS_SDK_LINE))
SILABS_SDK_DIR := silabs_tools/$(SILABS_SDK_LINE)

REQUESTED_SILABS_SDK_LINE := $(if $(strip $(SDK_LINE)),$(SDK_LINE),$(SILABS_SDK_LINE))
REQUESTED_SILABS_SDK_VERSION := $(call silabs_sdk_version,$(REQUESTED_SILABS_SDK_LINE))
...
assert-silabs-sdk-selector:
	@if [ -z "$(SILABS_SDK_LINE)" ] || [ -z "$(SILABS_SDK_VERSION)" ]; then \
		echo "Unsupported Silabs MCU '$(MCU)' for BOARD '$(BOARD)'. Update board.mk selector." >&2; \
		exit 1; \
	fi

assert-requested-silabs-sdk-line:
	@if [ -z "$(REQUESTED_SILABS_SDK_LINE)" ] || [ -z "$(REQUESTED_SILABS_SDK_VERSION)" ]; then \
		echo "Unsupported Silabs SDK line '$(SDK_LINE)' requested." >&2; \
		exit 1; \
	fi
...
print-silabs-sdk-install-manifest:
	@printf '%s|%s|%s|%s\n' \
		"simplicity_sdk" \
		"$(call silabs_sdk_version,simplicity_sdk)" \
		"$(call silabs_sdk_archive,simplicity_sdk)" \
		"$(call silabs_sdk_url,simplicity_sdk)"
	@printf '%s|%s|%s|%s\n' \
		"gecko_sdk" \
		"$(call silabs_sdk_version,gecko_sdk)" \
		"$(call silabs_sdk_archive,gecko_sdk)" \
		"$(call silabs_sdk_url,gecko_sdk)"
